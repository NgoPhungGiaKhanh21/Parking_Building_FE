...
import { enrichCheckoutSession, isWalkInDriverLookup } from "../../../utils/plateLookupUtils";
import {
...
const VehicleExit = () => {
  ...
  /** Driver reservation checkout — lookup API does not return checked-in reservations. */
  ...
  const isDriverWalkIn = useMemo(() => {
    if (isDriverReserved || !lookupSession) return false;
    if (isWalkInDriverLookup(plateLookup)) return true;
    return isDriverWalkInSession(lookupSession, plateLookup);
  }, [isDriverReserved, lookupSession, plateLookup]);

  const activeSession = useMemo(() => {
    if (driverReservation) return driverReservation;
    return lookupSession;
  }, [driverReservation, lookupSession]);
  ...
  }, [activeSession, plateInput, plateLookup, payments, driverReservation]);
  ...
  const isGuest = Boolean(lookupSession) && !isDriver;
  ...
  const sessionLoading = lookupLoading || reservationsLoading;
