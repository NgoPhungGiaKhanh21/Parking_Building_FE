...
import {
  ...
  buildDriverPlateIndex,
  isKnownDriverPlate,
  getDriverInfoFromPlateIndex,
...
const VehicleExit = () => {
  ...
  const driverPlateIndex = useMemo(
    () => buildDriverPlateIndex(reservationList),
    [reservationList],
  );
  ...
  const normalizedGuestSession = useMemo(
    () => (guestSession ? normalizeReservation(guestSession) : null),
    [guestSession],
  );

  const isDriverReserved = !!driverReservation;
  const isDriverWalkIn = useMemo(() => {
    if (isDriverReserved || !normalizedGuestSession) return false;
    if (isDriverWalkInSession(normalizedGuestSession)) return true;
    return isKnownDriverPlate(
      normalizedGuestSession.vehiclePlate || plateInput,
      driverPlateIndex,
    );
  }, [isDriverReserved, normalizedGuestSession, plateInput, driverPlateIndex]);
  ...
  const normalizedSession = useMemo(() => {
    ...
    const fromReservation = getDriverInfoFromPlateIndex(plate, driverPlateIndex);
    ...
    return mergeDriverInfo(base, {
      driverUsername: cached?.driverUsername || fromReservation?.username,
      driverFullName:
        cached?.driverFullName ||
        fromReservation?.fullName ||
        fromPayment?.driverName ||
        fromPayment?.fullName,
    });
  }, [activeSession, plateInput, driverPlateIndex, payments]);
  ...
  const sessionErrorMessage =
    typeof sessionError === "string" ? sessionError : sessionError?.message || null;
